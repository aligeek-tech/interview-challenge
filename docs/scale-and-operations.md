...
Due-work selection uses a stable database statement timestamp for indexed eligibility; expiry then samples the actual database clock after all business locks before deciding. The publisher uses `SKIP LOCKED` for outbox claims. Background expiry uses it on the voyage root, then follows voyage → booking → hold order with a transaction-local lock timeout for later waits. It never claims a hold first. A stable `(expires_at, hold_id)` keyset cursor advances across polls, including past busy candidates, and wraps after each cutoff-bounded sweep. New deadlines enter a subsequent sweep.

## Measurements needed to prove the design

Structured logs, business and delivery audit rows, version-aware readiness, worker activity, pending/quarantined counts and oldest backlog ages form the implemented baseline. The following names are a **production instrumentation plan**, not a claim that a metrics exporter is installed:
...
Automatic poison quarantine and an audited local redrive command are implemented; see [recovery runbook](recovery-runbook.md). Data pruning, broker administration, production alert delivery and capacity repair remain separate operational work. Retain inbox and request-idempotency evidence for the documented replay/retry windows; do not delete it merely to reduce table size. Database backup/restore, clock synchronization, replica failover and broker outage procedures need separate operational verification.

## Executable worker bounds and health

Each expiry worker uses at most one item transaction at a time. Defaults are 64 candidates per page, 256 examined candidates and 100 successful expirations per poll, a one-second poll budget and a 50-millisecond per-lock timeout. Busy voyage IDs are deduplicated within a poll. A worker-local traversal cursor prevents older blocked batches hiding later voyages; it grants no ownership and losing it cannot lose work. Repeated time-budget exhaustion without resolving any candidate is reported as a worker failure. Small-scale regression fixtures prove cross-voyage progress and cancellation bounds; they are not a throughput benchmark.

Each publisher claims one message before delivery and holds no claim connection during transport I/O. Its local consumer opens a separate transaction. More replicas multiply these budgets; choose total pool limits against measured database headroom. The single hot-voyage lock remains the serialization point. Backpressure/admission limits for 5,000 offered hold requests per second remain a design decision to set from load measurements, not a capability established by these tests.

Readiness checks use a two-second database deadline. Enabled workers must complete successful polls or durable item progress; an unrelated heartbeat timer cannot refresh them. Empty successful polls are healthy, disabled workers are explicit, and caught failures or stalled polls are unhealthy. Activity age uses monotonic process time; business deadlines and leases still use database time. The default freshness threshold is 30 seconds and must exceed the configured poll interval.

The health JSON reports pending messages, oldest pending age, quarantine count, overdue active holds and oldest expiry lag. Quarantine or backlog older than two minutes is Degraded with HTTP 200 when schema/workers remain ready: durable bookings can still be accepted during a transport outage. A stalled required worker or incompatible schema gives readiness 503. Liveness stays independent; backlog alone is not a restart policy. Probe counts may be expensive on a large backlog; the bounded timeout makes that failure visible, and production should move aggregates to sampled telemetry before increasing scale.
