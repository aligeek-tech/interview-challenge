...
./scripts/dotnet.sh run --project src/CapacityBooking.Api --no-build -- --migrate
./scripts/dotnet.sh run --project src/CapacityBooking.Api --no-build -- --seed
./scripts/dotnet.sh run --project src/CapacityBooking.Api --no-build --urls http://127.0.0.1:5080
```
...
`--migrate` applies embedded, ordered SQL files in one transaction under a PostgreSQL advisory lock. Applied migration checksums prevent silently editing migration history. Application startup does not automatically change the schema. `/health` checks database access and migration-table availability.
...
| `GET /health` | 200 when database and migration table are accessible; otherwise 503 |
...
- The worker durably reconciles elapsed deadlines. GET reports a past-deadline active row as effectively expired without writing. Until a worker or mutating operation processes it, its units remain conservatively reserved. No overselling occurs during this lag.
